...
public class AudioVolumeEditWindow : EditorWindow
{
    ...
    Dictionary<string, BeachModel> UIAudioGroup;
    Dictionary<string, BeachModel> SceneAudioGroup;
    //序列化对象
    ...
    {
        UIAudioGroup = new Dictionary<string, BeachModel>();
        SceneAudioGroup = new Dictionary<string, BeachModel>();
        ...
        Dictionary<string, BeachModel> audioDic = new Dictionary<string, BeachModel>();
        if (FileInfo.Exists)
        {
        ...
        //UI
        foreach(object music in Enum.GetValues(typeof(GlobeThus.UIMusic)))
        {
        ...
        // Scene
        foreach (object music in Enum.GetValues(typeof(GlobeThus.SceneMusic)))
        {
            string name = music.ToString();
            if (name == "None") continue;
            BeachModel model = new BeachModel();
    ...
    public Dictionary<string, BeachModel> LoadJson(string Filepath)
    {
        StreamReader reader = new StreamReader(Filepath);
        string jsonString = reader.ReadToEnd();
        Dictionary<string, BeachModel> audioGroup = JsonMapper.ToObject<Dictionary<string, BeachModel>>(jsonString);
        reader.Dispose();
        reader.Close();
        return audioGroup;
    }
    ...
    {
        Dictionary<string, BeachModel> dic = new Dictionary<string, BeachModel>();
        MergeDictionaries.Chunk(dic, UIAudioGroup);
        MergeDictionaries.Chunk(dic, SceneAudioGroup);
