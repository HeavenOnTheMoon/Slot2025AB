...
public class BuildEditScript : MonoBehaviour
{
    ...
    public static void Check(int buildTargetIndex, BuildInfoData info)
    {
        ...
        if (Resources.FindObjectsOfTypeAll<SpyLimeYet>()[0].BaseHow != info.BaseUrl)
        {
            isSame = false;
            Debug.Log("域名");
        }
...
#endif
        if (Resources.FindObjectsOfTypeAll<ADVaguely>()[0].MAX_SDK_KEY != info.Applovin_SDK_KEY)
        {
            isSame = false;
            Debug.Log("admanager key");
        }
        if (Resources.FindObjectsOfTypeAll<ADVaguely>()[0].MAX_REWARD_ID != info.Applovin_REWARD_ID)
        {
            isSame = false;
            Debug.Log("reward id");
        }
        if (Resources.FindObjectsOfTypeAll<ADVaguely>()[0].MAX_INTER_ID != info.Applovin_INTER_ID)
        {
            isSame = false;
            Debug.Log("inter id");
        }
        if (Resources.FindObjectsOfTypeAll<SpyLimeYet>()[0].FistFend != info.GameCode)
        {
            isSame = false;
            Debug.Log("gamecode");
        }
    ...
    public static void SuperBuild(int buildTargetIndex, BuildInfoData info)
    {
        ...
        Resources.FindObjectsOfTypeAll<ADVaguely>()[0].MAX_SDK_KEY = info.Applovin_SDK_KEY;
        Resources.FindObjectsOfTypeAll<ADVaguely>()[0].MAX_REWARD_ID = info.Applovin_REWARD_ID;
        Resources.FindObjectsOfTypeAll<ADVaguely>()[0].MAX_INTER_ID = info.Applovin_INTER_ID;
        ...
        if (!GameObject.Find("MainManager") || !GameObject.Find("MainManager").GetComponent<CausalWoolVaguely>())
        {
            if (GameObject.Find("MainManager"))
            {
                GameObject.Find("MainManager").AddComponent<CausalWoolVaguely>();
            }
            else
            {
                GameObject mainObj = new GameObject("MainManager");
                mainObj.AddComponent<CausalWoolVaguely>();
            }
        }
        GameObject.Find("MainManager").GetComponent<CausalWoolVaguely>().UnevenID = info.Adjust_APP_ID;

#if UNITY_IOS
        GameObject.Find("MainManager").GetComponent<CropDyVaguely>().Belle = info.Rate_ID;
#endif
#if UNITY_ANDROID
        GameObject.Find("MainManager").GetComponent<CropDyVaguely>().appid = info.PackageName;
#endif
        Resources.FindObjectsOfTypeAll<SpyLimeYet>()[0].FistFend = info.GameCode;
        Resources.FindObjectsOfTypeAll<SpyLimeYet>()[0].BaseHow = info.BaseUrl;
        Resources.FindObjectsOfTypeAll<SpyLimeYet>()[0].LeftAlongHow = info.BaseUrl + CLazily.LoginHow;
        Resources.FindObjectsOfTypeAll<SpyLimeYet>()[0].LeftLazilyHow = info.BaseUrl + CLazily.LazilyHow;
        Resources.FindObjectsOfTypeAll<SpyLimeYet>()[0].LeftTheyHow = info.BaseUrl + CLazily.TheyHow;
        Resources.FindObjectsOfTypeAll<SpyLimeYet>()[0].LeftCausalHow = info.BaseUrl + CLazily.CausalHow;
        EditorSceneManager.SaveScene(EditorSceneManager.GetActiveScene());

#if UNITY_IOS
        string url = info.BaseUrl + CLazily.LazilyHow + info.GameCode + "&channel=" + "AppStore" + "&version=" + buildWindow.Version;
#elif UNITY_ANDROID
        string url = info.BaseUrl + CLazily.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#else
        string url = info.BaseUrl + CLazily.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#endif
