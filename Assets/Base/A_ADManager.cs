...
public class A_ADManager : MonoBehaviour
{
    ...
    // 广告加载状态
    [HideInInspector] public bool hasRewardAdLoaded = false;
    public Transform NoAdTip;
    ...
    public void playRewardVideo(Action<bool> OnRewardAdCompleted)
    {
        ...
        {
            if (NoAdTip.transform.localPosition.x >= 800)
            {
                NoAdTip.transform.DOLocalMoveX(-800, 3).SetEase(Ease.Linear).OnComplete(() =>
                {
                    NoAdTip.transform.localPosition = new Vector2(800, NoAdTip.transform.localPosition.y);
                });
            }
