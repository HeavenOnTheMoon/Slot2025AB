...
public class EpidermisPalimpsest : MonoBehaviour
{
    void Start()
    {
    ...
    public static void WhatID(GameObject IconImage, float IconMoveYStart, float IconMoveYFinal,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void InkFair(GameObject PopBarUp,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void InkFund(GameObject PopBarDisapper,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void GatherLittle(int startNum, int endNum,float delay, Text text,System.Action finish)
    {
    ...
    public static void GatherLittle(double startNum, double endNum, float delay, Text text, System.Action finish)
    {
        GatherLittle(startNum, endNum, delay, text, "", finish);
    }
    public static void GatherLittle(double startNum, double endNum, float delay, Text text, string prefix, System.Action finish)
    {
        DOTween.To(() => startNum, x => text.text = prefix + LittleJean.SummerMyNut(x), endNum, 0.5f).SetDelay(delay).OnComplete(() =>
        {
            finish();
        });
    }

    public static void OrbitID(float startNum, float endNum,int nowLevel,int fullCount , float delay, Image image,Text text, System.Action finish)
    {
    ...
    /// <param name="finish"></param>
    public static void GatherCoerceAdult(float startValue,float endValue,Image sliderImage, System.Action finish, float delay = 0)
    {
    ...
    /// <param name="finish"></param>
    public static void LiveFair(GameObject HomeTopBarShow, float MoveOffsetShow, System.Action finish) 
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    /// <param name="MoveOffset"></param>
    /// <param name="finish"></param>
    public static void LiveFund(GameObject HomeTopBarHide, float MoveOffsetHide, System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void VinePeak(GameObject GoldImage,Transform parent, int a ,Transform GoldStart,Transform GoldFinal,System.Action finish) 
    {
        if (a == 0)
        {
            finish();
    ...
    public static void VinePeakAkin(GameObject GoldImage, int a, Transform StartTF, Transform EndTF, System.Action finish)
    {
        //如果没有就算了
        if (a == 0)
        {
        ...
        for (int i = 0; i < a; i++)
        {
            ...
            GoldIcon.transform.localScale = new Vector3(0.8f, 0.8f, 0.8f);
            //金币弹出随机位置
            ...
            s.Append(GoldIcon.transform.DOMove(new Vector3(GoldIcon.transform.position.x + OffsetX, GoldIcon.transform.position.y + OffsetY, GoldIcon.transform.position.z), 0.15f).SetDelay(Delaytime).OnComplete(() =>
            {
                ...
                if (t % 5 == 0)
                {
                    GlobeYet.LawFigurine().ArchAdvent(GlobeThus.UIMusic.Sound_GoldCoin);
                }
            }));
            //金币移动到最终位置
    ...
    /// <param name="i"></param>
    public static void UnitHomePeak(GameObject GameBankCashImage,Transform parent,Transform BankCashStart,Transform BankCashFinal,int i,System.Action finish)
    {
        ...
        if (i <= 2)
        {
            ...
            GameBankCash.transform.DOMoveY(BankCashFinal.position.y,0.7f).OnComplete(()=>{
                ...
                finish();
                //GlobeYet.GetInstance().PlayEffect(GlobeThus.SceneMusic.Sound_GameBankGold);
            });
        }
        else if( i==3 )
        {
            ...
            GameBankCash.transform.DOMoveY(BankCashFinal.position.y, 0.7f).OnComplete(() => 
            {
                ...
                //GlobeYet.GetInstance().PlayEffect(GlobeThus.SceneMusic.Sound_BankCashFull);
    ...
    /// <param name="finish"></param>
    public static void TineBoxPeak(GameObject StarBox,float BoxMoveOffset,GameObject Fx_StarBoxOpen, System.Action RewardShow) 
    {
    ...
    /// <param name="finish"></param>
    public static void TineLeoMeadowFair(List<GameObject> CardList, int Count, System.Action finish)
    {
        ...
        float screenW = UIInject.LawFigurine().WarmGenius.GetComponent<CanvasScaler>().referenceResolution.x;
        int finishCount = Count;
        if (Count == 3) 
        {
            float Space1 = 80f;
            float Space2 = ((UIInject.LawFigurine().WarmGenius.GetComponent<CanvasScaler>().referenceResolution.x - Space1 * 2) - CardW * Count) / (Count - 1);
    ...
    /// <param name="finish"></param>
    public static void OrbitRacialITooSpeed(GameObject UnLevelIcon, Sprite LevelIcon, Text text, System.Action finish)
    {
    ...
    /// <param name="finish"></param>
    public static void DenseCharmHat(GameObject box,GameObject Fx_RewardBG,System.Action finish) 
    {
    ...
    /// <param name="i"></param>
    public static void Hatchet(GameObject BankBtn,int i) 
    {
        float offset = -0.6f;
    ...
    public static void WhatMilk(GameObject IconImage)
    {
    ...
    public static void OVOPeak(GameObject GoldImage, Transform parent, int a, Transform GoldStart, Transform GoldFinal, System.Action NextShow, System.Action finish)
    {
        if (a == 0)
        {
            finish();
    ...
    /// <param name="Finish"></param>
    public static void EquivalentSierra(GameObject obj, float addPosition, System.Action Finish)
    {
    ...
    public static void GatherDeclare(double startNum, double endNum, float delay, Text text, Action finish = null)
    {
