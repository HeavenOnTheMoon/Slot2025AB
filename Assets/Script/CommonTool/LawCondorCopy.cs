...
{
    public static void Chunk<TKey, TValue>(this IDictionary<TKey, TValue> first, IDictionary<TKey, TValue> second)
    {
    ...
    public class LawCondorCopy : EarnCommunity<LawCondorCopy>
{
    // Start is called before the first frame update
    public bool VastAgency;
    public bool BeMeal;

    static public LawCondorCopy instance;
    ...
    /// <returns></returns>
    public Vector3 JobYouthBrace(GameObject gameobj)
    {
    ...
    /// <returns></returns>
    public GameObject LawCuriousFistSource(Transform SelfTransform,List<Transform> listTemp)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public float BraceMySkull(Vector2 p1, Vector2 p2)
    {
        Vector2 p;
        p.x = p2.x - p1.x;
        p.y = p2.y - p1.y;
    ...
    /// <returns></returns>
    public float Skull180My360(float angle)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public int LawLedgePeregrine(Vector2 p1, Vector2 p2)
    {
        float angle = Skull180My360(BraceMySkull(p1, p2));
    ...
    /// <returns></returns>
    public Vector2 JobSurviveTrueTomb(Renderer Renderer)
    {
    ...
    public Vector2 JobSurviveTrueTomb(Image image)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public Vector2 JobDuringTomb(GameObject obj)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector2 JobToxicCommerce()
    {
        
        Vector3 v = Input.mousePosition;
        float xa = v.x / Screen.width;
        float ya = v.y / Screen.height;
        float screenW = JobFollowWedge();
        float screenH = JobFollowCobalt();
        float x = -screenW/2 + xa * screenW;
        float y = -screenH/2 + ya * screenH;
        return new Vector2(x, y);   
    }

    public int JobHeroicMagic(List<string> weightList)
    {
        float sumWeight = 0;
    ...
    public int[] LawMortalSlippery(int total, int count)
    {
    ...
    public Dictionary<string, object> ExcessMyNationwide(string value)
    {
        if (value.Length < 1)
        {
            return null;
    ...
    public string NationwideMyExcess(Dictionary<string, object> Info)
    {
    ...
    public int CowCanySash(string dateStart, DateTime dateEnd)
    {
    ...
    public int JetCanySash(string dateStart, DateTime dateEnd)
    {
    ...
    public TimeSpan CanySash(DateTime dateStart, DateTime dateEnd)
    {
    ...
    public Vector3 LawSeeAtMine(Canvas canvas, Text text, string strFragment)
    {
        int strFragmentIndex = text.text.IndexOf(strFragment);//-1表示不包含strFragment
        Vector3 stringPos = Vector3.zero;
        if (strFragmentIndex > -1)
        {
            Vector3 firstPos = LawSeeAtMine(canvas, text, strFragmentIndex);
            Vector3 lastPos = LawSeeAtMine(canvas, text, strFragmentIndex + strFragment.Length);
            stringPos = (firstPos + lastPos) * 0.5f;
        }
        else
        {
            stringPos = LawSeeAtMine(canvas, text, strFragmentIndex);
        }
        return stringPos;
    }

    public Vector3 LawSeeAtMine(Canvas canvas, Text text, int charIndex)
    {
    ...
    /// <returns></returns>
    public float EventualSoSuck(Vector3 pB, Vector3 pA1, Vector3 pA2)
    {
    ...
    private static byte[] Gene= { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    /// <summary>
    /// DES加密字符串
    /// </summary>
    ...
    public static string ConsistDES(string encryptString, string key = "abcdefgh")
    {
        try
        {
    ...
    public static  string GeneralDES(string decryptString, string key = "13717421")
    {
        byte[] rgbKey = Encoding.UTF8.GetBytes(Application.identifier.Substring(0, 8));
        byte[] rgbIV = Gene;
